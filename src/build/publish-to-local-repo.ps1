...
function Push-NugetPackageToLocalRepo() {
    param (
        [Parameter(Mandatory = $false)][String]$LocalNugetRepo = "local",
    ...
    _ExecSafe { & dotnet nuget push "$SolutionDir/src/Basalt.UniversalFileSystem/bin/$Configuration/Basalt.UniversalFileSystem.$PackageVersion.nupkg" --source $LocalNugetRepo }
    _ExecSafe { & dotnet nuget push "$SolutionDir/src/Basalt.UniversalFileSystem.Cli/bin/$Configuration/Basalt.UniversalFileSystem.Cli.$PackageVersion.nupkg" --source $LocalNugetRepo }
}